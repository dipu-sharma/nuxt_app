      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <main class="lg:col-span-9">
          ...
          <div v-if="!query && !hasSearched" class="mb-10">
            <div class="mb-10" v-if="trending.length">
              <h2 class="text-[10px] font-bold uppercase tracking-widest text-text opacity-50 mb-4 flex items-center gap-2">
            ...
            <h2 class="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-text to-text/60 tracking-tight">Popular Products</h2>
          </div>

          <!-- Results count -->
          <div v-if="hasSearched" class="mb-8 text-sm text-text opacity-70 font-semibold bg-secondary/30 inline-block px-4 py-2 rounded-full border border-border/50">
            {{ products.length }} results found for <span class="text-primary font-bold">"{{ query || selectedCategory || 'Filters' }}"</span>
          </div>

          <!-- Loading State -->
          <div v-if="loading" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
            <div v-for="i in 6" :key="i" class="bg-card/60 backdrop-blur-xl rounded-[2rem] overflow-hidden animate-pulse border border-white/20 shadow-lg">
          ...
          <div v-else-if="hasSearched && products.length === 0" 
            ...
            <p class="text-text opacity-60 text-lg max-w-sm mx-auto">Try refining your keyword query or resetting categories.</p>
          </div>
          ...
          <div v-else>
            ...
            <div v-if="hasMore" class="mt-16 text-center">
              <button @click="loadMore" :disabled="isLoadMoreLoading" class="relative group overflow-hidden rounded-[1.5rem] font-bold text-sm tracking-widest uppercase text-white shadow-xl hover:-translate-y-0.5 hover:shadow-2xl hover:shadow-indigo-500/30 transition-all h-14 px-12 inline-flex items-center justify-center disabled:opacity-50 disabled:pointer-events-none">
                <div class="absolute inset-0 bg-gradient-to-r from-indigo-600 via-purple-600 to-cyan-500 transition-transform duration-500 group-hover:scale-105"></div>
