<template>
  <aside class="z-20 flex-shrink-0 hidden w-64 overflow-y-auto bg-black dark:bg-gray-800 md:block">
    <div class="py-4 text-gray-500 dark:text-gray-400">
      <div class="py-2 px-8">
        <h1 class="items-center text-xl text-bold justify-center text-white">Admin Dashboard</h1>
      </div>
      <ul class="mt-6">
        <li class="relative px-6 py-3" v-for="(item, index) in sidebarItems" :key="index">
          <NuxtLink
            class="inline-flex text-center items-center w-full text-sm font-semibold transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200"
            :to="item.path"
            :class="{ 'text-black bg-white p-2 rounded-lg text-center items-center': isActive(item.path) }">
      ...
      <ul class="">
        <li class="relative px-6 py-3">
          <button
            ...
            <svg v-if="isMobileUsersMenuOpen" class="w-4 h-4" aria-hidden="true" fill="none" stroke-linecap="round"
              stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
              <path d="M19 9l-7 7-7-7"></path>12
            </svg>
            <svg v-if="!isMobileUsersMenuOpen" class="w-4 h-4" aria-hidden="true" fill="none" stroke-linecap="round"
              stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <ul class="mt-2 space-y-2">
              <li>
                <NuxtLink to="/admin/users/"
                  ...
                  :class="{ 'text-black bg-white rounded-md p-2': isActive('/admin/users/') }">
                  User
                </NuxtLink>
              </li>
              <li>
                <NuxtLink to="/admin/users/doss"
                  class="inline-flex items-center w-full text-sm font-semibold transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200 p-2"
                  :class="{ 'text-black bg-white rounded-md p-2': isActive('/admin/users/doss') }">
                  Doss
                </NuxtLink>
              </li>
              <li>
                <NuxtLink to="/admin/users/merchants"
                  class="inline-flex items-center w-full text-sm font-semibold transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200 p-2"
                  :class="{ 'text-black bg-white rounded-md p-2': isActive('/admin/users/merchants') }">
                  Merchants
                </NuxtLink>
              </li>
            </ul>
          ...
          <ul v-if="navStore.isSideMenuOpen" class="mt-2 space-y-3 text-center items-center">
            <li v-for="(item, index) in usersMenuItems" :key="index"
              ...
              <NuxtLink
                ...
                :to="item.path" @click="activeIndex = index">
                <span class="ml-4">{{ item.label }}</span>
              </NuxtLink>
            </li>
          </ul>
