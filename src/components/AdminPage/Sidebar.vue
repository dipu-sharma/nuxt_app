  ...
  >
    <div class="py-4 text-gray-500">
      ...
      <ul class="mt-6">
        ...
        >
          <NuxtLink
            class="inline-flex text-center items-center w-full text-sm font-semibold transition-colors duration-150"
            :to="item.path"
            :class="{
              'p-2 rounded-lg text-center items-center': isActive(item.path),
              'bg-black text-white':
                themeStore.theme === 'light' && isActive(item.path),
              'bg-white text-black':
                themeStore.theme === 'dark' && isActive(item.path),
            }"
          >
            <svg
              class="w-5 h-5"
      ...
      <ul class="">
        <li class="relative px-6 py-3">
              ...
              <li>
                <NuxtLink
                  ...
                  :class="{
                    ...
                    'text-white bg-black':
                      themeStore.theme === 'light' &&
                      isActive('/admin/users/merchants'),
                    'text-black bg-white':
                      themeStore.theme === 'dark' &&
                      isActive('/admin/users/merchants'),
                  }"
          ...
          <ul
            v-if="navStore.isSideMenuOpen"
            class="mt-2 space-y-3 text-center items-center"
          >
            <li
              v-for="(item, index) in usersMenuItems"
              :key="index"
              :class="[
                'relative px-6 py-3 items-center',
