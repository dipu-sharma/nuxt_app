        ...
        <div v-if="isOpen"
            ...
            <v-row>
                ...
                <v-col cols="12" md="8">
                    <div v-show="tab === 0" class="flex gap-4">
                        <v-text-field type="date" label="Start Date" v-model="form.start_date" clearable class="tab-1"
                            variant="outlined" />
                        <v-text-field type="date" clearable label="End Date" v-model="form.end_date" class="tab-1"
                            variant="outlined" />
                    </div>
                    <div v-show="tab === 1">
                        <v-select label="Type" v-model="form.type" :items="typeOptions" clearable variant="outlined" />
                    </div>
                    <div v-show="tab === 2">
                        <v-select label="Executive" v-model="form.exicutive_id" clearable :items="executiveOptions"
                            variant="outlined" />
                    </div>
                </v-col>
            </v-row>
            ...
            <v-row>
                <v-col cols="12" class="flex justify-center gap-4">
                    <button class="hover:bg-blue-400 hover:text-white px-4 py-1 rounded-lg bg-blue-300 text-white"
                        @click="applyFilter">Apply</button>
                    <button class="hover:bg-red-400 hover:text-white px-4 py-1 rounded-lg bg-red-300 text-white"
                        @click="dialog = false">Close</button>
                </v-col>
            </v-row>
        </div>
    </v-slide-x-reverse-transition>
